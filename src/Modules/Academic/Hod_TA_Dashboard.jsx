...
export function Hod_TA_Dashboard() {
  ...
  useEffect(() => {
    ...
    axios
      .get(URLS.TA_LIST_URL, { headers: { Authorization: `Token ${token}` } })
      .then((r) =>
        setTAs(
          r.data.tas.map((t) => ({
    ...
    axios
      .get(URLS.FACULTY_LIST_URL, {
        headers: { Authorization: `Token ${token}` },
      })
      .then((r) =>
        setFacs(
          r.data.faculties.map((f) => ({
  ...
  function loadStipends() {
    const token = localStorage.getItem("authToken");
    axios
      .get(URLS.HOD_PENDING_URL, {
        headers: { Authorization: `Token ${token}` },
      })
      .then((r) => setPending(r.data.stipends));
    axios
      .get(URLS.HOD_APPROVED_URL, {
        headers: { Authorization: `Token ${token}` },
      })
